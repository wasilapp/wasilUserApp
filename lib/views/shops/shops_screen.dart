...
class ShopsScreen extends StatefulWidget {
  ...
  const ShopsScreen({super.key, required this.lat,required this.long,required this.id});
...
class _ShopsScreenState extends State<ShopsScreen> {
  ...
  new GlobalKey<RefreshIndicatorState>();  final GlobalKey<RefreshIndicatorState> refreshIndicatorKey =
  new GlobalKey<RefreshIndicatorState>();

  @override
  Widget build(BuildContext context) {

    return RefreshIndicator(key:refreshIndicatorKey,
   ...
   buildAddress(),
              ...
              ListView.builder(itemBuilder: (context, index) =>  buildShop(controller.shops[index]),
  ...
  //
  //     items: shops,
  //     label: ' Search name store',
  //     menuMode: true,
  //     menuHeight: 100,
  //     hideSearch: false,primaryColor: AppColors.primaryColor,
  //    decoration: BoxDecoration(borderRadius: BorderRadius.circular(8),border: Border.all(color: AppColors.primaryColor,)),
  //     prefixIcon: const Padding(
  //       padding: EdgeInsets.all(0.0),
  //       child: Icon(Icons.search),
  //     ),
  //     dropDownMenuItems: shops!.map((item) {
  //
  //       return item['shop_name_en'];
  //     }).toList() ,
  //
  //     onChanged: (value) {
  //
  //       UserNavigator.of(context).push(SubCategoryScreen(shopId: value['id']));
  //       // if (value != null) {
...
Widget buildShop(ShopsModel shop){
        ...
        Container(
          ...
          child: ListTile(
            ...
            height: 80,
              child: Stack(
                  children: [
                    Image.network('https://news.wasiljo.com/${shop.avatarUrl}',width: 70,
                      height: 80,),
                    Container(
                      width: 70, // نفس حجم الصورة
                      height: 80,
                      color:    shop.open==0?  Colors.black.withOpacity(0.3):Colors.transparent, // لون الغطاء مع الشفافية
                    ),
                    shop.open==0?  Center(child: Text("closed".tr,style: TextStyle(fontSize: 15,color: Colors.white,fontWeight:FontWeight.bold )),
                    ):Text('')

                    // Container(width: 70,
                    //   height: 80,
                    //  decoration: BoxDecoration(borderRadius: BorderRadius.circular(10),
                    // color:    shop.open==0?  Colors.grey.shade900:Colors.transparent,
                    //            ),),
                    // child:
 ...
 //             height: 80,
 //            decoration: BoxDecoration(borderRadius: BorderRadius.circular(10),
 //           color:    shop.open==0?  Colors.grey:Colors.transparent,
 //            image: DecorationImage(image:
 // NetworkImage('https://news.wasiljo.com/${shop.imageUrl}'),            ),),
 //             child: shop.open==0? const Center(child: Text("busy",style: TextStyle(fontSize: 15,color: Colors.white,fontWeight:FontWeight.bold )),
 //
 //             ):const Text(""),
 //
 //            ),
              ...
              children: [
                Column(mainAxisAlignment: MainAxisAlignment.start,crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      Row(children: [
                        const Icon(Icons.star,color: AppColors.primaryColor,size: 15),
                        Text(
                ...
                shop.open==0?   Text("closed".tr,style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),):const Text("")
              ],
            ),
            onTap: () {
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Obx(() =>      addressController.selectedAddress.value.length==0&& addressController.listDefaultAddress.isEmpty? Text(' select Address'.tr):addressController.selectedAddress.value.length==0?
...
print('object');
                  ...
                  itemBuilder: (BuildContext context) {
                    print('object');
                    addressController.getMyAddresses();
                    final items = addressController.listAddress.map((address) {
...
print(address.longitude!.toString());
                          ...
                          addressController.lat.value=address.latitude!.toString();
                          addressController.long.value=address.longitude!.toString();
                          addressController.street.value=address.street;
                          addressController.buildingNumber.value=address.buildingNumber!;
                          addressController.apartmentNum.value=address.apartmentNum!.toString();
